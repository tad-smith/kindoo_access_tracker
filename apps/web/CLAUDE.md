...
- **`standardCallings.ts` is a known copy of a shared table, and the conformance test is what keeps it honest.** The request typeahead's lists reproduce `@kindoo/shared`'s `CALLING_ORDER` — 92 names in the same order — because that table is module-private and `callingSortOrder` exports a lookup, which cannot be enumerated. `tests/standardCallings.test.ts` pins the copy: every entry resolves, the concatenation ascends strictly by `callingSortOrder`, and the covered indices are gap-free. Never weaken those assertions to make a shared-table edit pass; update the copy. Deriving instead of copying is T-97. The ward / branch split on top of it is built by **subtraction** from a `UNIT_CALLINGS` union, so only the two hide-sets need editing when the shared table grows.
