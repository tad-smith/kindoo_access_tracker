...
//
//   - `siteIdForScope(scope, wards, buildings)` — resolves a submission
//     scope (a ward_code or `'stake'`) to the Kindoo site id that scope
//     targets. Stake-scope resolves to `null` (home only — per spec §15
//     Phase 2, stake-scope is intentionally restricted to home-site
//     buildings). Ward-scope resolves through the ward's building
//     (`resolveWardSite`); `null` = home. Returns `null` if the ward
//     isn't in the catalogue yet (live subscription not hydrated).
...
//   - `siteLabelForSeat(seat, wards, buildings, sites)` — returns the
 ...
 * field absent; treat that as home (`null`). Empty-string is not a
 * valid id from the Configuration UI but we coerce it for defensiveness.
 */
 ...
 * `KindooSite.display_name` when the seat's scope is a ward whose
 * building is bound to a foreign site; `null` otherwise (stake-scope,
 * home-site wards, unknown wards, or sites collection not yet hydrated).
 */
 ...
 * view layer) against the sites catalogue, falling back to resolving
 * the scope's ward through its building on un-migrated legacy data
 * (where the grant's `kindoo_site_id` is null).
 *
 ...
 * resolving the seat's `scope` through its ward's building when the
 * field is absent (legacy / pre-migration data still in flight).
 * Returns `null` for home; a string id for foreign.
 *
 * Externally-visible behaviour matches the pre-T-42 ward path — the
 * field is just a denormalisation. Centralised here so every caller
 * (utilization filters, roster badges, stake-pool exclusion) reads the
 * same fallback chain.
 */
export function seatSiteId(
  ...
  // Field absent: fall back to the ward path so legacy seats
  // (pre-migration) still resolve correctly.
  return siteIdForScope(seat.scope, wards, buildings);
}
