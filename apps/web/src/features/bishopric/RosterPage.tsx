...
export function BishopricRosterPage({ initialWard }: BishopricRosterPageProps) {
  ...
  // the matched-grant band rather than the primary's. Auto-band
  // intra-sort still keys on the seat's top-level `sort_order` —
  // `DuplicateGrant` has no `sort_order` field, so a duplicate-matched
  // auto row sorts at the primary's calling rank. See spec §15 Phase B
