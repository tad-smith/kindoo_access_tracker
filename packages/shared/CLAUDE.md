...
├── stakeTime.ts            # stake-timezone calendar helpers (D20, D37)
├── tempExpiry.ts           # expired-temp-grant rule + syncWillClearSeat (D34, D37)
├── scheduledTasks.ts       # scheduled-task shapes + schedule arithmetic (D38)
└── index.ts
```
...
- **A schedule is a closed set of shapes, not a cron string.** `scheduledTasks.ts` owns `TaskSchedule` and all of its arithmetic; the dispatcher wakes hourly, so an expression that could ask for anything finer would be silently inert (D38, `spec.md` §17).
