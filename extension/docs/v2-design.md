...
6. **Description merge format** — synthesized from the SBA seat's primary grant + each `duplicate_grants[]` entry:
   ...
   - **`scopeName` comes from `resolveScopeName(scope, stake, wards)`** (`content/kindoo/provision.ts`), not from the raw scope value. Stake scope → `stake.kindoo_expected_site_name || stake.stake_name`. Unit scope → the ward doc matching `ward_code`, run through `kindooScopeName(ward.ward_name)` (`@kindoo/shared`, `architecture.md` D31): a **ward** gains the canonical `" Ward"` suffix if its stored name lacks it, a **branch** (name ends in `" Branch"`) is written verbatim with nothing appended. No matching ward doc → the raw scope string, verbatim. Getting this wrong is not cosmetic: the provisioner's description comparison is a strict `!==`, so a scope name that doesn't match what Church Access Automation writes makes every pass rewrite the Description, and the read-side `parseDescription` stops resolving the segment.
