...
**A duplicate-surfaced row offers no destructive fix.** `projectSeatForSite` reports whether the grant it projected was the seat's primary or a `duplicate_grants[]` entry; the detector carries that as `surfacedFromDuplicate`, and `fixActionsFor` withholds `sba-only`'s **Remove From SBA** and `callings-mismatch`'s **Update SBA** on such rows (the row still renders, and its reason says why). Every `syncApplyFix` handler writes the PRIMARY's fields, so those two would destroy a grant the operator isn't looking at — B-24 and B-16 respectively. `scope-mismatch` / `type-mismatch` / `buildings-mismatch` stay available: same class, but they reshape an axis of a grant that still exists rather than destroying one. Lift the withholding when the handlers take the surfaced grant's `(scope, kindoo_site_id)`.
