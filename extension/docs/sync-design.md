...
| no seat **projected onto this site** | Kindoo user present, created-seat type derivable (temp / admin / Guest-with-grant / non-blank Description) | `kindoo-only` (drift — Create SBA seat; **Add SBA grant** when a seat doc exists whose grants all live on other sites — B-23) |
...
3. **`deriveOverlappingRuleIds`** — pure function. A user claims a rule iff their door set shares **at least one** DoorID with it. **A member is in a building if they can open any of its doors** — one door is a way in, and there is no partial entry. Empty rule door sets are explicitly guarded against (would otherwise vacuously match every empty rule). Used for BOTH building sets; only the door subset differs (all doors → `derivedBuildings`, church-granted doors → `directGrantBuildings`).
4. **`deriveEffectiveRuleIds`** — pure function, strict subset: a rule is claimed iff EVERY DoorID in it is present. **Not used by Sync** — its one caller is `provision.ts`, deciding whether writing an AccessRule would be redundant, which is the question "would this rule add a door", not "can this member get in". Reading access with this predicate was B-25; see below.
...
**A duplicate-surfaced row offers no fix that would write the primary.** `projectSeatForSite` reports whether the grant it projected was the seat's primary or a `duplicate_grants[]` entry; the detector carries that as `surfacedFromDuplicate` on **every** projected row, and `fixActionsFor` withholds the action, with the row's reason carrying a note so a missing button is never the only signal. **`duplicateGuard.ts` holds the set and that note — it is the source of truth; do not restate the membership here or in `spec.md`.** As of this writing it is `callings-mismatch`, `scope-mismatch`, `type-mismatch`, `buildings-mismatch` and `kindoo-unparseable` — every code whose handler writes the PRIMARY's fields, which on such a row is not the grant the operator is looking at (B-16). **`sba-only` is deliberately NOT withheld:** since B-24 its payload names the surfaced grant's `(scope, kindooSiteId)` and `applySbaOnlyRemove` drops that duplicate, so it is the removal path for everything a `kindoo-only` merge creates — withholding it left merged auto grants undeletable, because every web Remove affordance gates on `grant.type !== 'auto'`. Lift the rest when their handlers take the same discriminator.

**The `kindoo-only` merge refuses a slot hit already on the grant's site**, soft-failing with "that grant is already recorded on this Kindoo site". The detector cannot emit a row in that state, so it has no false positives; it catches a second manager clicking an already-applied row, and an out-of-date extension sending the unfiltered `primaryScope` before the Web Store publishes the matching build.
...
|---|---|---|
| `sba-only` | "Remove From SBA" (danger) | SBA-side delete via `syncApplyFix` with `code: 'sba-only'`, payload `{ memberEmail, scope, kindooSiteId }` — the last two name the grant the row was surfaced from (B-24), taken from the row's `sba` block, which IS the projection. Three branches: **drop-duplicate** when the named scope isn't the primary's (remove that `duplicate_grants[]` entry, rebuild `duplicate_scopes`, primary untouched, **no access reap** — a duplicate-only scope never had an `importer_callings` entry); **plain `tx.delete`** for a primary-surfaced orphan with no duplicates; **promote-first-duplicate-to-primary** when a primary-surfaced row's seat carries duplicates. A payload with no `scope` (an extension predating B-24) skips the first branch, giving the historical delete-or-promote. (Was a Kindoo-side "Provision in Kindoo" write before the Kindoo-authoritative shift.) |
| `kindoo-only` | "Create SBA seat", or "Add SBA grant" when the member already has a seat (`mergesOntoExistingSeat`) | SBA-side: `syncApplyFix` with `code: 'kindoo-only'`. Server-side stamps the seat write with `SyncActor:kindoo-only`. Creates the seat when the member has none; **merges** onto it when they do (B-23) — slot by `(scope, type)`, primary then `duplicate_grants[]`, union buildings + re-stamp the slot's `kindoo_site_id` on a hit, append a parallel-site `DuplicateGrant` on a miss. The primary's identity is never modified. Same payload either way — one code, one action, two labels. |
...
>
> **The same predicate was also wrong for access, which is the destructive half.** The rule is
> operator-stated and flat: *a member is in a building if they can open any of its doors — they do
> not need all of them.* A door you can open is a way in; there is no partial entry. Under the strict
> subset a member holding some of a building's doors claimed nothing from the ALL-doors set either,
> so `derivedBuildings` omitted a building they can walk into — and `buildings-mismatch` sources its
> Update SBA from `derivedBuildings`, so the offered fix was to write that member's `building_names`
> down to `[]`. The first pass at B-25 fixed only the provenance side and argued the split — "access
> needs every door, provenance needs one" — which is wrong on the access half and left the fix
> action pointed at a building the member holds.
>
> Both sets are now `deriveOverlappingRuleIds` over their respective door subsets: all doors →
> `derivedBuildings`, church-granted doors → `directGrantBuildings`. `deriveEffectiveRuleIds`
> survives for `provision.ts` alone, where the question really is "would writing this rule add a
> door". A grant on a door in no mapped rule counts for nothing on either side — the operator's
> ruling: buildings are read through the rules SBA models, not through Kindoo's door list at large.
> The residual is over-claim where two rules share a door; physical doors belong to one building, so
> that takes a rule selecting another building's door — a mapping error this surfaces rather than
> hides.
...
**Algorithm** (`deriveOverlappingRuleIds`, restricted to church-granted doors — the same predicate
`derivedBuildings` uses, over a narrower door subset):
...
church-granted door subset — the same any-overlap predicate `derivedBuildings` runs over all doors
(B-25);
