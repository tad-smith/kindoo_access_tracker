...
- **Auth flow — two paths, both owned by `lib/auth.ts`, both landing on one Firebase session.** They are alternatives a manager picks between in `SignedOutPanel`, never a fallback chain.
  - **Google (`signIn`):** `chrome.identity.getAuthToken` → `GoogleAuthProvider.credential(null, accessToken)` → `signInWithCredential`. Persists the Google access token alongside the principal snapshot.
  - **Web handoff (`signInViaWeb`):** `chrome.identity.launchWebAuthFlow` on `${VITE_WEB_BASE_URL}/auth/extension?redirect_uri=<chrome.identity.getRedirectURL()>` → the SPA redirects back with `#token=<Firebase custom token>` → `signInWithCustomToken`. This is the **only** path open to a manager with no Google account, so it must never be gated behind the Google one. It carries a **custom** token, not the SPA's ID token, because an ID token expires in an hour and only the context holding the refresh token can renew it — the SPA's auth window is gone by then, and a relayed ID token would strand the extension. `signInWithCustomToken` mints the extension its own refresh token, which is what survives SW suspends. Read the **fragment**, never a query string. Persists only the principal snapshot — there is no Google access token on this path, and writing one would be a lie the sign-out path acts on.
  - **Failure mapping.** The SPA never redirects on a cancelled sign-in, so a closed auth window arrives as a `launchWebAuthFlow` failure via `chrome.runtime.lastError` — mapped to `consent_dismissed`. Every `#error=<code>` is `sign_in_failed`, **including unrecognised codes**: a future SPA code an installed build predates must degrade to a hard error, not fall through to "success with no token."
  - **`consent_dismissed` is worded per path** (`SignedOutPanel`'s `dismissedCopy`), and the split is load-bearing rather than cosmetic. A closed window and a `redirect_uri` the SPA refused produce the *identical* bare `lastError`, so the web path cannot claim a cancellation and must not order a retry — the SPA's refusal card explains the fault inside the auth window and says retrying is pointless, and that window is gone by the time the panel renders. Copy that commanded a retry would overwrite the manager's only explanation and loop them on the one action that cannot work. The Google path keeps the plain "Sign-in cancelled" wording; Chrome's consent dialog has no failure state to explain. Don't collapse the two back into one string. `buildWebAuthUrl` logs the redirect URI before launching for the same reason — it is the only place the refused value is observable from this side. **The phrase "configuration error" inside the web copy is shared wording with the SPA's refusal card**, not a phrasing choice: the panel points the manager back at the card, and the card names the same phrase. Reword the surrounding sentence freely; changing that phrase on either side alone leaves the pointer naming words the manager never saw. The card is the referent and this copy is the reference, so if the two ever drift, the card's wording is the one to match. A test on each side pins it.
  - **`signOut()` serves both.** Its cached-Google-token probe is best-effort and resolves `undefined` when there is none, falling through to `firebaseSignOut`. Don't make that probe blocking — a manager who cannot sign out is worse off than one who cannot sign in.
  - The SW keeps Firebase Auth state across suspends via the SDK's IndexedDB persistence; the slim principal snapshot is also persisted to `chrome.storage.local`.
...
- **Remote apply** (`docs/architecture.md` D27) lets a manager tap a pending request on their phone and have this extension provision it. Opt-in per profile ("Allow requests from my phone", top of the Queue tab). While opted in, the content script heartbeats presence and polls `remoteApply/{canonicalEmail}/jobs` for `queued` work — both through SW handlers, both one-shot. Nine properties are load-bearing:
  ...
  - **Don't let both provisioning entry points run at once.** `RequestCard`'s button is disabled while a phone-initiated job holds that `request_id` (`remoteApplyBusy`, threaded from `QueuePanel` via `useRemoteApply`). Two concurrent `applyRequest` runs mean two `inviteUser` calls for a member not yet in Kindoo — a consumed licence. **`running` is only this tab's own claim**, so the gate takes a second input, `onBusyRequestIds`: every `request_id` a job holds. It buys four things — a queued job this tab **cannot** serve (another tab will claim it), a queued job waiting behind the one being run, a job **another tab is running**, and **this tab's own claim until its terminal write lands**. That last one is not redundant with `running`: `onJobEnd` fires from a `finally` whether or not the job reached a terminal status, so dropping the id at claim time reopened the button from an exhausted `finishWithRetry` until the next successful poll re-read the job as `running` — and a `partial` leaves the request `pending`, so its card is still on screen to be clicked. **Publish the filtered set only after the terminal write, never at the claim.** `QueuePanel` lets `running` name the phase, so the overlap costs nothing. It does NOT buy the phone's tap → claim window for a job this tab CAN serve: the poll that first sees such a job claims it in the same tick, so the gate is held across one Firestore write, not a poll period. That window belongs to the phone, which withholds its own Apply button for a request with a live job. The set is **not** filtered by `canClaimRemoteApplyJob` (every job in the mailbox is this manager's), and it is **two reads, in this order**: the queued page first, then `runningJobs` — a job only ever moves `queued → running`, so that order catches it on one side or the other, where running-first leaves a hole the width of the two reads. Don't collapse the second read into the sweep's: the sweep is rate-limited to 60s against a 10s poll, and it runs ahead of the session gate, so its list is both stale and on the wrong side of the ordering. **Every tick that can't make those two reads holds the last answer, and holds it on one clock.** A failed `runningJobs` read, a tick that ends before the poll (no Kindoo session, no usable heartbeat), and a tick that throws are all the same question — "I couldn't look; is a sibling still working?" — so none of them may publish `[]` on no evidence. The transient failures are the reachable ones: `getEnvironments` now gives up after `KINDOO_REQUEST_TIMEOUT_MS` instead of hanging, and `no-eid` is a DOM scrape that returns null on zero *or several* matches — and the tick's failure and the manager's click are independent samples, so the button can go live and succeed precisely because the condition was transient. The hold is **bounded** at `REMOTE_APPLY_BUSY_HOLD_MS` (150s), measured from the last poll where BOTH reads landed (`lastBusyReadAt`, written in one place): above two hidden-tab poll periods and above the ~90s a single failed `getEnvironments` blinds a tab for, below `REMOTE_APPLY_STRANDED_MS`, past which the sweep has already disowned any `running` job the gate would be citing. Don't add a second timer beside it, and don't reintroduce an unbounded hold — a gate nothing can lift strands a manager whose own button is dead while no sibling is serving them. The bound is only total while **every** exit from a tick publishes something, which is the second reason a denied presence write is caught rather than thrown (above). For the same reason `publishedBusyIds` starts at **`null`, not `[]`**: the dedupe is loop-local where the React host's state is not, so a loop restarted by a `stakeId` change must be able to emit its first empty set instead of having it deduped against a set the host still holds forever. **The opt-in check is the one path that clears outright**, because opting out stops the loop (so a held set would have no later tick to age it out) and the flag is profile-wide (so no sibling tab is polling either). `inFlight` is excluded from the `running` read because this tab's own run is bracketed by `onJobStart` / `onJobEnd` and by the claimed id the poll holds. Reject stays enabled; it makes no Kindoo write.
...
│       ├── auth.ts                # both sign-in paths: chrome.identity Google
│       │                          # credential exchange + the launchWebAuthFlow
│       │                          # handoff to the SPA (SW)
...
- **Every Kindoo call is bounded.** `postKindoo` arms an `AbortController` at `KINDOO_REQUEST_TIMEOUT_MS` (30s) across the headers *and* the body read, and reports the abort as `KindooApiError('network-error', '… timed out …')` — the same code a dropped connection gets, because both leave "did this reach Kindoo?" equally unanswerable. `fetch` waits forever otherwise, and the remote-apply tick is serialised: one hung call means no heartbeat, no stranded sweep, and for a single-tab manager a claimed job left `running` with nothing alive to finalise it. The value is a per-CALL bound and a provision is sequential (~9 calls worst case, ~270s), so it is chosen to keep a wholly-degraded run as near `REMOTE_APPLY_STRANDED_MS` as a per-call bound can — not inside it. The run's two SBA callables carry the Functions SDK's own 70s default apiece, which this constant cannot shorten, so the worst case is ~410s and a sibling tab can sweep a run that is still live. That is the residual D27(k) accepts; raising this constant is what turns it from a worst case into the ordinary shape of a degraded run.
