...
Status: in progress (tracks a/b/c/e DONE 2026-05-30 — PRs #178 sort, #179 detector, #180 backend shaping merged to main; track d PENDING)
...
- (b) **@backend-engineer** — (1) `firestore.rules` allowance for the new optional `ward.building_id` field on ward create/update (test: "manager write of a ward carrying `building_id` + `building_name` is allowed"; existing name-only writes still allowed). (2) One-time **ward-backfill migration script**: derive `building_id` from each ward's `building_name` (via `buildingSlug` / by matching the building doc) and write it onto existing ward docs. Migration is **required**. (3) Audit any Cloud Function that uses `ward.building_name` as an FK and make it id-first where appropriate. `building_name` stays on wards for now — dropping it is a deliberate later follow-up, not part of this PR.
- (c) **@docs-keeper** — `docs/spec.md` (ward→building reference is now id-first; building display name decoupled from slug, names must be unique) + `docs/firebase-schema.md` §4.2 (`ward.building_id` field) and §4.3 (slug immutability on building edit).
