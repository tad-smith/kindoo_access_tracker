...
## [T-?] Correct `architecture.md` D33(d) — the pattern check is no longer the security boundary
Status: open
Owner: @docs-keeper
Phase: cross-cutting

Number deliberately unassigned — `docs-keeper` assigns it. Filed by `web-engineer` on `feat/ext-auth-route` alongside the code change, because D33 now records a security property the code no longer has, in the direction that invites someone to "fix" the code back.

**What is wrong.** D33(d) states the `redirect_uri` pattern check is "the whole security boundary" and argues it "does not distinguish this extension from another one the user has installed, and it does not need to — … completing the handoff requires the user to sign in inside the window that opened it." That premise does not hold. A manager who already holds an SPA session on the origin signs in to nothing: the route minted on mount. So any extension in the profile holding only the `identity` permission could run `launchWebAuthFlow({ interactive: false })` against `/auth/extension` and receive a custom token — no window, no click, nothing observable — exchangeable via `signInWithCustomToken` (the web API key ships in the bundle) into a full session with its own renewable refresh token, indistinguishable from the manager. Found by the PR #282 automated reviewer.

**What the code does now** (see `spec.md` §4.1, "Two gates guard the mint"):
1. `isAllowedRedirectUri` (`apps/web/src/features/auth/extensionRedirect.ts`) tests the captured extension ID against an allowlist — `CHROME_EXTENSION_ID` plus additive `VITE_EXTENSION_IDS` for unpacked dev builds. The shape regex is now only a precondition.
2. The mint fires from an explicit "Connect the extension" click, never on mount. A non-interactive flow renders no UI and so can never produce it. The confirm names the account and offers "Use a different account".

**Also needs rewriting: D33's rejected-alternatives note.** It rejects "an allow-list of known extension IDs in place of the pattern" on the grounds that differing staging / prod / unpacked IDs would need an env-specific SPA deploy. The env var settles that — the published ID is compiled in and works everywhere with no per-env value; only unpacked dev builds set anything. The allowlist is also not "in place of" the pattern; it composes with it.
