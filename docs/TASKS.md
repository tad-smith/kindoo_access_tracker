...
- `apps/web/src/features/requests/standardCallings.ts` — reconciled three overlapping-entry spellings to match the authoritative sort table (`Aaronic Priesthood Specialists`→`…Specialist`, `Young Women Specialists`→`…Specialist`, `Temple and Family History Leader`→`Ward Temple and Family History Leader`). Scanned the rest; the other near-misses (`Young Women Advisors`, `Stake History Specialist`, `Stake Single Adult Adviser`) are distinct callings, not spelling variants of a table entry, so left alone, as are standardCallings' extras (Patriarch, Building Representative, etc.) and its list order. (standardCallings is suggestion-only per its header — renames are safe.)
