...
**Bounded by wall-clock as well as attempt count**, found in review and the sharper half of the fix. `clearEmulators` runs in `afterEach` under vitest's default 10s `hookTimeout` — no config in the repo sets one and no hook overrides it — and the retried condition is *itself* a timeout (`Transaction lock timeout`), so a failing attempt is not necessarily fast. An attempt-only bound could therefore blow the hook budget and report `Hook timed out in 10000ms` against whatever unrelated test was running, losing the attempt count and body the retry exists to produce: the same misattributed diagnosis, one layer up. An 8.5s budget now keeps the SWEEP from being the half that overruns the hook (the Auth half carries no deadline of its own — the Admin SDK calls take no signal — so this is not a whole-hook guarantee; it measures ≤84ms, so it is not treated as urgent), enforced on each request via `AbortSignal.timeout` rather than only on the decision to start another attempt — otherwise one slow answer overruns the hook however few attempts were made, which is precisely the case that motivated the bound. The same `try`/`catch` also brings transport failures (a socket reset) into the retry; they previously escaped as a bare `TypeError` with no attempt count.

**Thrown failures are classified, not blanket-retried** — the third review round, and a genuine regression the first cut introduced. Treating every throw as transient meant that with the Firestore emulator gone but Auth still up (separate processes, so the Auth half succeeds and the sweep is still reached), each `ECONNREFUSED` cost four attempts and ~700ms; across the ~500 `clearEmulators` calls in a run that is **~6 minutes of pure backoff added to an already-red run**, under a 20-minute cap, where it used to throw in ~0ms. Now only the deadline abort and dropped connections retry; `ECONNREFUSED` and programming errors fail on the first attempt.

**The retryable code set is measured, not recalled.** The first cut listed `ECONNABORTED` / `EPIPE` / `ETIMEDOUT` — Node socket errnos that **undici never emits**, so the branch read as thorough and matched almost nothing, and its test passed only because the fixture hand-built a cause. Measured on Node 22.22.2: a refused connection is `ECONNREFUSED`, an RST is `ECONNRESET`, a graceful FIN is `UND_ERR_SOCKET`, and a keep-alive reuse after the server closed is `UND_ERR_HEADERS_TIMEOUT`. `UND_ERR_SOCKET` is the realistic one — ~500 sequential DELETEs over a pooled connection to a Java emulator with its own idle timeout is exactly that setup — and it used to fall through to fail-fast. The set is now `ECONNRESET` + `UND_ERR_SOCKET`. `ECONNREFUSED` and `UND_ERR_CONNECT_TIMEOUT` stay out, since the connection was never established; undici's own `UND_ERR_HEADERS_TIMEOUT` / `UND_ERR_BODY_TIMEOUT` stay out because they are **unreachable from this call site** — they default to 300s and every attempt carries an `AbortSignal.timeout` of ≤8.5s, so the abort always fires first. Measured both ways: the keep-alive reuse that yields `UND_ERR_HEADERS_TIMEOUT` under a bare `fetch` yields `TimeoutError` once the signal is attached. Listing them would have repeated the original mistake in a subtler form — plausible codes that never arrive.

**And the wrapper no longer swallows the cause.** undici's message for a transport failure is the information-free `fetch failed`; the address and errno live on `err.cause`. Wrapping it dropped both, so the most common real failure of this helper — running `test:integration` with the emulator down — went from vitest printing `[cause] Error: connect ECONNREFUSED 127.0.0.1:8080` to reporting `TypeError: fetch failed` and nothing else. A diagnostic regression on the fail-fast path, in a change whose whole thesis is not losing the diagnostic, and hitting far more often than the 409 this task is named for. The errno is folded into the message and the original attached as `cause`. A latch backs that up: once one sweep exhausts its budget **without the emulator ever answering**, the rest short-circuit — the same reasoning `waitForDelivery` uses, and bounded per test file rather than per run, since vitest isolates module state.

That "without ever answering" qualifier is load-bearing, and the fourth review round caught its absence. `outOfTime` is reachable in two attempts while the emulator *is* answering, because the retried condition is a lock **timeout** — a contended DELETE only returns `Transaction lock timeout` after already waiting for the lock. At ~5s per attempt: attempt 1 answers 409 at t≈5s, `outOfTime` is still false, attempt 2 aborts at 8.5s, latch set. Every remaining hook in that file then failed with "the emulator is not answering" — untrue, and naming nothing. One failed test became a failed file, in exactly the scenario this task documents: the same misattributed diagnosis T-97 was filed about, made wider. Gating on `lastResponse === undefined` separates a hung or absent emulator from a slow one and keeps the protection where it belongs. Tightened twice more. It arms only when the sweep saw **no contact at all** — neither an HTTP response nor a transport drop. A dropped connection means one existed, which is evidence of life just as a 409 is. This has to be a sticky flag rather than a check on the terminal error, and the reason is subtle enough that two earlier attempts got it wrong: with a per-attempt `AbortSignal`, a flappy socket usually does *not* terminate as a drop. Once the remaining budget is shorter than the drop delay the abort wins the race, so the last error is a `TimeoutError` with no response recorded — indistinguishable from silence unless the earlier drops are remembered. The test that was supposed to cover this passed only because its stub ignored `init.signal`, modelling a race that cannot happen. And the flag has to be set *before* the body is read: `res.text()` streams under the same signal, so a 409 whose body lands near the deadline aborts mid-read, and recording contact afterwards would count a demonstrably-answering emulator as silence. And it needs **two consecutive** silent sweeps, not one: from inside a single call, a contended DELETE slower than the budget is indistinguishable from a hung emulator, and the quantity that would decide it is the one this code openly does not know. Arming on one would red the rest of the file with a wrong cause — the misattribution T-97 is about, widened. Two in a row keeps the wall-clock bound while making a misfire need two consecutive >8.5s silences. A successful sweep resets the count, and the message now reports what was observed rather than diagnosing the emulator.

**Knowingly asymmetric, and recorded as a decision rather than left implicit.** An emulator that 409s every sweep costs ~700ms × ~500 calls ≈ 6 minutes — the identical runaway `ECONNREFUSED` is failed fast to avoid — and nothing bounds it, since the latch requires no response at all. Left that way because a strike does not cost the same on both sides. `ECONNREFUSED` is unambiguous: nothing is listening, and every later call is certain to fail identically. Two contended sweeps in a row are not — trigger fan-out bursts make that reachable in an otherwise healthy run, and short-circuiting there would convert a transient into a file-wide failure, replacing N independent named failures with one. For a case never yet observed, paying the 6 minutes on an already-red run is the better side of that trade. Revisit if it is ever seen.

The old wording here claimed the fail-fast branch was exercised by "a 400 from a malformed URL". It is not: a malformed URL throws `TypeError: Failed to parse URL`, which landed in the catch and was retried four times — the single example given for fail-fast was the one case that did not take it. Both are now real, and both are tested. How long a *contended* DELETE actually takes is still unknown — the single sighting only bounds it under 10s — which is exactly why the bound is time-based rather than a tuned attempt count.

**The budget is measured, not guessed** — the second thing review caught. It was 5s on no evidence, which in the 5–10s slow-contention band would have turned a sweep that blocks and then *succeeds* (a pass today, under the 10s hook) into a red abort: worse behaviour in precisely the regime the bound targets. Timed against this emulator, the Auth half costs ≤84ms and an uncontended sweep ≤50ms, so the hook normally finishes in ~0.1s of its 10s. 8.5s keeps the abort a last resort and leaves ~1.5s for vitest to report. `clearEmulators` subtracts the Auth half's actual elapsed time before handing the remainder to the sweep, so a slow Auth half shrinks the sweep's slice instead of pushing the pair over the hook.

**An abort no longer discards the diagnostic.** An abort is always terminal — it fires *at* the deadline, so there is never time for another attempt — and its message says only "aborted due to timeout". The last real response is carried into the final error, so it still names the `Transaction lock timeout` body that explains why the sweep was slow. Without that, the slow-contention path produced a bare `TimeoutError`: strictly less informative than the 409 this task was filed about.
...
Eighteen unit tests in `functions/tests/lib/emulator.test.ts` cover first-try success, retry-then-succeed, bounded give-up on attempt count, give-up on the wall-clock deadline across many slow attempts, abort of a SINGLE attempt that outlives the deadline (the case an attempt-count bound cannot catch), retention of the 409 body when the terminal attempt is an abort, retry of a dropped connection (`ECONNRESET`, and `UND_ERR_SOCKET` — the shape a pooled connection actually produces), fail-fast on `ECONNREFUSED` with the address preserved, fail-fast on a malformed URL, no-retry-on-400, the latch short-circuiting a later sweep after a non-answering emulator, the latch NOT firing when the emulator answered 409 (the T-97 case itself), the latch NOT firing on repeated transport drops, the latch needing two consecutive silent sweeps, a success resetting the strike count, the errno surviving an empty `cause.message` (the `localhost` AggregateError shape), and — the one that pins the most consequential line — repeated 409-then-abort sweeps never latching, which is the only case reaching the guarded branch at all, since every other test terminates on the attempt ceiling where `outOfTime` is false. They stub `fetch`, so they need no emulator — the transient path a healthy emulator never takes.
