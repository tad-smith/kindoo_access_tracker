...
- **Cloud Tasks dedupe.** Deterministic task id per (stake, job, hour), with `task-already-exists` caught as success. Cloud Tasks holds a used id for roughly an hour after execution, which matches the dispatch cadence exactly. This is the guard for the overlap hazard inherited from [T-103], below.
