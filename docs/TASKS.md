...
3. **Importer fan-out.** Integration test against a multi-site LCR fixture: a person with callings on both home and at least one foreign site, processed by the importer, produces a seat whose top-level `kindoo_site_id` matches the primary's site and whose `duplicate_grants[]` includes one entry per other site the person has a calling on, each carrying that site's `kindoo_site_id`, the per-site calling list, and per-site `building_names`. Within-site priority losers continue to land with `kindoo_site_id === primary.kindoo_site_id` (no regression).
4. **`Seat.kindoo_site_id` written top-level.** Every importer-produced seat doc (and every seat doc written by `markRequestComplete`) carries `kindoo_site_id` at top level: derived from primary scope + ward (stake-scope ⇒ home). Verified by unit test + integration test.
5. **Provision orchestrator per-site writes.** Given a seat whose primary + duplicates span N distinct `kindoo_site_id` values, the orchestrator emits exactly N Kindoo writes (one per site), each using the matching Kindoo session and writing the union of `building_names` from grants on that site. Within-site priority losers do not produce a separate write. The plan is computed upfront; if any required site has no active Kindoo session / missing `kindoo_eid` / EID mismatch, the orchestrator refuses the entire write with a clear error before any write fires (no partial-provision states). Verified by extension integration test with a multi-site fixture seat.
6. **Request-completion auto-merge stamps `kindoo_site_id`.** A `markRequestComplete` call that auto-merges a new manual / temp grant onto an existing seat's `duplicate_grants[]` stamps `kindoo_site_id` on the new entry, derived from the request's scope and ward lookup. The path is `planAddMerge` in `functions/src/callable/markRequestComplete.ts:113`. Verified by Cloud Functions integration test.
7. **One-shot migration.** Running the migration callable over a fixture stake with N seats (mix of home, foreign-only, multi-site) populates `kindoo_site_id` on every seat doc and every `duplicate_grants[]` entry. Idempotent — second run produces no diffs. Missing-ward `duplicate_grants[]` entries are skipped with a logged warning (no error, no "home" fallback). The migration uses a dedicated `migration_backfill_kindoo_site_id` audit action code. The callable takes a `stakeId` parameter.
