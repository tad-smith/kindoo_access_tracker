...
Fourteen unit tests in `functions/tests/lib/emulator.test.ts` cover first-try success, retry-then-succeed, bounded give-up on attempt count, give-up on the wall-clock deadline across many slow attempts, abort of a SINGLE attempt that outlives the deadline (the case an attempt-count bound cannot catch), retention of the 409 body when the terminal attempt is an abort, retry of a dropped connection (`ECONNRESET`, plus `UND_ERR_SOCKET` and `UND_ERR_HEADERS_TIMEOUT` — the shapes a pooled connection actually produces), fail-fast on `ECONNREFUSED` with the address preserved, fail-fast on a malformed URL, no-retry-on-400, the latch short-circuiting a later sweep after a non-answering emulator, and the latch NOT firing when the emulator answered 409 (the T-97 case itself). They stub `fetch`, so they need no emulator — the transient path a healthy emulator never takes.
