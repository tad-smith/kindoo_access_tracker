...
**Sequencing.** T-43 has two hard prerequisites — distinct events, both must land before T-43 rolls out:
1. **T-42 Phase A code lands** (`feat/t-42-phase-a-implementation` merged to main). Brings the schema, importer fan-out, orchestrator per-site walk, `markRequestComplete` `kindoo_site_id` stamping, and the one-shot migration callable.
2. **T-42 Phase A migration callable run on production** (and any other live stakes). Without this, every existing seat's top-level `kindoo_site_id` and each `duplicate_grants[]` entry's `kindoo_site_id` are `undefined`, so the `isParallelSite` predicate is always false — Phase B's behaviour degrades to today's primary-only rendering (graceful no-op, not a misclassification, but Phase B is meaningless until the data is populated).
