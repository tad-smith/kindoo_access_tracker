...
Sixteen unit tests in `functions/tests/lib/emulator.test.ts` cover first-try success, retry-then-succeed, bounded give-up on attempt count, give-up on the wall-clock deadline across many slow attempts, abort of a SINGLE attempt that outlives the deadline (the case an attempt-count bound cannot catch), retention of the 409 body when the terminal attempt is an abort, retry of a dropped connection (`ECONNRESET`, and `UND_ERR_SOCKET` — the shape a pooled connection actually produces), fail-fast on `ECONNREFUSED` with the address preserved, fail-fast on a malformed URL, no-retry-on-400, the latch short-circuiting a later sweep after a non-answering emulator, the latch NOT firing when the emulator answered 409 (the T-97 case itself), the latch NOT firing on repeated transport drops, the latch needing two consecutive silent sweeps, a success resetting the strike count, and the errno surviving an empty `cause.message` (the `localhost` AggregateError shape). They stub `fetch`, so they need no emulator — the transient path a healthy emulator never takes.
