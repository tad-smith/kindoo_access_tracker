...
Two semantics worth not re-litigating. Matching is on the **scope-name portion** of a segment, exact and case-insensitive — a substring rule would swallow a calling like `Aspen (Maple Ward Liaison)`. And only **unresolved** segments are eligible, which makes the list structurally incapable of hiding one of our own wards even if an entry collides with a ward name after a later rename; the UI guard is then a better error rather than the only defence.
