...
## [T-68] Building rename staleS display-name grant arrays (deferred)
Status: deferred
Owner: TBD
Phase: cross-cutting

Renaming a building is now an in-place edit on a frozen `building_id` slug, so the ward → building FK survives (T-67). But existing `seat.building_names` / `request.building_names` (display-name snapshots) and the extension's building-name → Kindoo-rule maps still point at the OLD name until the seat/request is re-saved. Follow-up: cascade a building rename into those grant-array references (a Sync sweep that rewrites stale `building_names`, or a rename-confirm warning). Deferred per operator 2026-06-05; grant arrays intentionally stay display-name (out of scope for T-67). Same item the automated reviewer flagged on the T-67 changelog (`docs/changelog/t-67-immutable-building-id-ward-slug.md`).
