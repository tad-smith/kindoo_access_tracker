...
- **Missed windows advance from the stored value, never from `now`.** `nextTriggerTime` starts at the stored `next_trigger_time` and advances one slot at a time until it lands in the future. That pins the trigger's phase — an `hourly` task seeded at :17 stays at :17 — where re-basing on `now` would walk it forward by however late each dispatch ran. It still fires once and re-bases rather than replaying every slept-through window, because the loop stops at the first future slot; the loop is bounded so a long-stale or hand-typed value cannot spin.
