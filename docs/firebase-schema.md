...
**The stamp is not retroactive.** Only entries inserted after D26 shipped carry it. An Elders Quorum President granted access earlier stays full — page, claim, and rules all agree, because all three read the same untouched record — until that scope is next written. `backfillEqPresidentAccess`'s **grant** direction skips a scope that already carries the calling, so a grant pass alone converts nobody; converting an existing president means a revoke pass followed by a grant pass. Nothing was backfilled at deploy, and a doc without the field computes a byte-identical claim block, so no refresh tokens churned.
