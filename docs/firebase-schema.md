...
**Read by:** the Configuration page's Sync Reminder card (`useStakeSchedule`), for the `syncReminder` row only — it renders the toggle, "not yet seeded" / off / on states, and a "next check" time. Every other row a stake might carry, and every other page, still reads nothing here.
