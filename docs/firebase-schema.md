...
- **Delete guard.** The Configuration tab blocks deleting an org while any **seat** references it via its primary `organization_id` OR any `duplicate_grants[].organization_id` (`organizationDeleteBlocker`, run against the live seats snapshot). Client-side only, same reasoning as the Kindoo-site / building guards.
