...
Active source of truth for the platform-superadmin role (Phase B / F19). The `syncSuperadminClaims` trigger reads writes here and mints / revokes the `isPlatformSuperadmin: true` claim. Writes remain console-only per Phase B operator decision #2 — there is no in-app UI for adding or removing superadmins, by design (the chicken-and-egg of "who creates the first superadmin?" plus the operator's preference to keep this surface small).
...
**Written by:** Firestore console (chicken-and-egg — no in-app management; see Phase B in `firebase-migration.md`).
...
**Written by:** the `createStake` Cloud Function callable (Phase B / F19) emits `action='create_stake'` rows on every successful stake-doc write. Future superadmin-management triggers (if added) write `add_superadmin` / `remove_superadmin` rows here; for now those actions are reserved enum values with no writer.
