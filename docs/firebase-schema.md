...
- **Server-side compute:** Cloud Functions only, for: email send (Resend), audit-log fan-in, custom-claims sync, FCM push fanout, callable wrappers (request-completion, `syncApplyFix`), and the scheduled-task system — one hourly `onSchedule` dispatcher plus one generic `onTaskDispatched` runner, over per-stake rows in `stakeSchedules/{stakeId}` (§3.5, `architecture.md` D38). **Its job registry carries one entry, `syncReminder` (`architecture.md` D39), the rest of the registry is still empty.** Mirror inventory in §7. Auto-seat ingestion runs through the Chrome extension's Sync feature (see `spec.md` §8), not a server-side importer.
...
**Written by:** the dispatcher (seeding rows, and — since T-107 (`architecture.md` D39) — stamping `last_trigger_time` / `next_trigger_time` inside a transaction rather than a blind overwrite, so a concurrent manager write survives a dispatch pass) and that stake's Kindoo Managers. The rules still permit a manager to write any field in the two-key envelope, including `schedule`, and that remains a Firestore console edit — but the client write paths that exist, the Configuration page's **Sync reminders** and **Quarterly access reviews** sliders (T-108, `architecture.md` D41), both touch `enabled` alone on their own row, through one job-parameterized mutation (`useSetScheduledJobEnabledMutation(job)`), and neither ever creates a row.

**Read by:** the Configuration page's Config tab, for the `syncReminder` and `manualSeatReview` rows only (`useStakeSchedule`) — the **Sync reminders** and **Quarterly access reviews** sliders (sub-rows nested under Email Notifications Enabled) each render their own switch and its loading / "not yet seeded" / off / on states off this one read; no scheduled time is displayed for either. Every other row a stake might carry, and every other page, still reads nothing here.
...
```typescript
{
  ...
  // for ALL of this stake's email links (all eight templates — `spec.md` §9), replacing
  ...
  // The second hidden operator-only escape hatch, same shape as the one above —
  // NO UI anywhere, set by hand in the Firestore console, never written by any
  // code path, and expected to be REMOVED again once the run it was set for has
  // been read. ABSENT or false ⇒ off. True ⇒ this stake's next quarterly
  // manual-seat review run (`spec.md` §9, `architecture.md` D42) sends every
  // scope's mail to the stake's active Kindoo Managers instead of to that
  // scope's real recipients, marks subject and body `[DRY RUN]`, and names the
  // intended recipients in the body. The real recipient rule (D41(c)) still
  // runs — its answer is REPORTED, not obeyed — and a scope whose real
  // recipient set is empty still sends, saying a real run would have reached
  // nobody. Also read by the dispatcher, through the job's own `skipJitter`
  // predicate, so a dry run fires at its slot instead of waiting out its 0–20h
  // jitter offset (§17, D42(c)). NOT repeatable: a dry run still stamps
  // `last_manual_seat_review_date` below, so it consumes the quarter and the
  // operator deletes that field by hand afterwards.
  // `notifications_enabled: false` still suppresses every send, dry run or not.
  ...
  // Stake-local `YYYY-MM-DD` on which the quarterly manual-seat review last went
  // out (`spec.md` §9, D41). Absent ⇒ never sent, so the next monthly check sends.
  // Cadence state, not condition state, unlike the field above: it is NEVER
  // deleted, because "once a quarter" must count an empty quarter too — deleting
  // it the way the sync reminder's stamp is deleted would turn a quarter with no
  // manual seats into a fresh first send the following month. Listed in
  // `BOOKKEEPING_FIELDS`, so stamping it fans no audit row and `lastActor` is
  // left alone.
...
```typescript
{
  ...
  church_granted_buildings?: string[] | null; // The subset of THIS grant's building_names whose door access the Church Access Automation grants directly, as observed from Kindoo's per-door GrantedBy data. The complement is what a Kindoo Manager added, and therefore what an edit_auto request may remove (spec §6.1 Policy B, architecture.md D43). TRI-STATE: absent / null means NEVER OBSERVED, not "none" — consumers read unknown as "assume all of building_names is Church-granted", the direction that locks the edit UI rather than unlocking it; [] is a real observation meaning the Church grants nothing on this grant. Deliberately NOT clamped to building_names — it records what Kindoo said, and a name here missing from building_names is a buildings-mismatch the Sync detector already surfaces. Server-written only, via syncApplyFix's church-buildings-mismatch fix (§8 / spec §8); no rules change was needed because seats.update already confines client writes to organization_id. No backfill job — a stake's seats get stamped by a manager clicking through the drift rows.
  ...
  duplicate_grants: Array<{
    ...
    church_granted_buildings?: string[] | null; // Identical semantics to the top-level field, for THIS grant. Provenance is per-slot because building_names is per-slot — a member can hold a Church-seeded home-ward grant beside a parallel-site grant a manager built by hand. Same tri-state; same server-only writer (syncApplyFix resolves the surfaced grant via resolveGrantSlot, B-16 / B-24).
...
| `dispatchScheduledTasks` | `onSchedule`, cron `0 * * * *`, `Etc/UTC` | **The only scheduled function.** Walks `stakes`; per stake, seeds a `stakeSchedules/{sid}` row for any registry job the stake lacks (§3.5), enqueues one Cloud Tasks task per due row onto the `runScheduledTask` queue, then stamps `last_trigger_time` / `next_trigger_time`. Enqueue-before-stamp, so delivery is **at-least-once** and every handler must be idempotent within its window; a deterministic task id per (stake, job, UTC hour) makes a same-hour double enqueue a dedupe hit rather than a double run. Never throws — per-stake and per-task failures log at ERROR and are counted into the summary logged as `dispatchScheduledTasks: done`, a string an infra metric matches on and alerts on the *absence* of. Pins `kindoo-app@`. **`SCHEDULED_JOBS` carries two entries: `syncReminder`** (`{type:'daily', hour: 6}`, `defaultEnabled: false`) **and `manualSeatReview`** (`{type:'monthly', day: 1, hour: 2}`, `jitterSeconds: 72_000`, `defaultEnabled: false`) — a run seeds both onto every stake and, once a manager enables one, enqueues it; every other registry slot is still empty. A job carrying `jitterSeconds` holds its enqueued task in the Cloud Tasks queue for a per-stake, per-job deterministic delay (`jitterDelaySeconds`, an FNV-1a hash reduced into the window) before delivery, via `scheduleDelaySeconds` — spreading a fan-out job's stakes across a window instead of bursting them all at one slot. A job may waive that delay for a stake through `ScheduledJob.skipJitter?: (stake) => boolean`; the **job** owns the predicate, so the dispatcher names no job's field, and a predicate that throws is logged at ERROR and read as "no skip". `manualSeatReview` waives it under `stake.manual_seat_review_dry_run` (§4.1, `architecture.md` D42). Every successful enqueue logs one INFO carrying `stakeId`, `job`, `taskId` and `delaySeconds` — `0` where `skipJitter` fired, the deterministic offset otherwise. The per-stake stamp write is now a transaction (`commitScheduleChanges`) that re-reads the document and re-applies only this pass's stamps and seeds, so a manager's concurrent `enabled` toggle survives. See `spec.md` §17, `architecture.md` D38 / D39 / D41. |
| `runScheduledTask` | `onTaskDispatched`, `maxAttempts: 3` | The one generic worker for every scheduled job. Takes `{stakeId, job}`, resolves the handler from the same registry, calls it with `(stakeId, now)`. A malformed payload or an unregistered job name logs at ERROR and **returns** — retrying either can only fail the same way; a handler that throws propagates so Cloud Tasks retries it. Generic on purpose: a function per job would put a deploy-shape change and a queue behind every scheduled feature. Pins `kindoo-app@`. Declares `secrets: [RESEND_API_KEY]` (T-107, `architecture.md` D39(g)) — registering the sync reminder made this the first scheduled handler that can send email, and Cloud Functions mounts a secret only onto the function that declares it, not onto the module a registered job happens to call. Any future job that sends email needs this same binding on the worker — the manual-seat review (D41) needed none of its own, since the binding was already here. |
...
**The quarterly manual-seat review adds no function either.** `sendManualSeatReviewIfDue(stakeId, now)` (`functions/src/services/ManualSeatReviewService.ts`, `architecture.md` D41) is registered as `manualSeatReview` in `SCHEDULED_JOBS` — `{type:'monthly', day:1, hour:2}`, `jitterSeconds: 72_000`, `defaultEnabled: false` — and turned on from the **Quarterly access reviews** slider beside the sync reminder's, both children of the same `ScheduledJobToggle` component. Per due run it reads `stakes/{sid}/seats` and `stakes/{sid}/access` whole (target-scale collections, filtered in memory rather than earning composite indexes), groups every manual grant by its own scope, and sends one email per scope through `EmailService`'s `notifyScopeManualSeatReview`, paced one second apart. It writes `stakes/{sid}.last_manual_seat_review_date` (§4.1) once at least one scope's mail sends or is suppressed by the kill-switch, withholding the stamp on a run where every send fails so the next month retries; never deletes it. No push category — email-only by design, not deferred. **`stakes/{sid}.manual_seat_review_dry_run` (§4.1, `architecture.md` D42) redirects a run's every mail to the stake's active Kindoo Managers** — the real recipient rule still runs and is reported rather than obeyed, a scope with no real recipients mails the managers anyway, the stamp is still written, and `notifications_enabled: false` still suppresses every send.
