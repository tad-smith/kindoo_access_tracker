...
- **Case (1) of the original reminder request is still deferred** — "no Sync in ≥ 7 days". Nothing records when a Sync ran, so it needs the extension to write a heartbeat, which puts it behind a Chrome Web Store release. T-103's body carries the reasoning.
