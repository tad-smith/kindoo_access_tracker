...
- **`nextTriggerTime` advances from the stored value, never from `now`.** That pins a trigger's phase — an `hourly` row seeded at :17 stays at :17. Re-basing on `now` is simpler and walks every trigger later forever, by however late each dispatch happened to run.
