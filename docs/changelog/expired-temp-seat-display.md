...
- **The manager's Edit couldn't work either.** Same premise: the Kindoo user is gone, so `provisionEdit` throws on lookup. Gated for every principal, no manager exemption.
