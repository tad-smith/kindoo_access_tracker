...
- **The requests `create` predicate narrows for a limited caller.** Type restricted to `add_temp` / `edit_temp` / `remove`; temp windows ≤ 90 days; ward-scope temp requests locked to exactly the ward's own building; `remove` only against a seat whose `type == 'temp'`. Last conjunct in the predicate, short-circuiting on `!isLimited(stakeId)`. Spec: §6, §6.1; schema §6, §6.1.
- **The New Request form and the Edit Seat dialog narrow to match.** Temporary is the only offered type (and the mount default, and the post-submit reset value); the 90-day cap is stated as helper text and validated on `end_date`; at ward scope the buildings checklist collapses to a read-only row naming the ward's building. Stake scope keeps the normal site-filtered checklist. Spec: §5.1, §5.2.
...
- **The App Access page gains an access-level selector and a Limited badge.** Full / Limited on the "Add manual access" modal; an `info` badge on grants carrying the marker, in both the table and card views. There is no edit-level control — re-tiering is delete-and-re-add. Spec: §5.3.
...
- **T-75 — the code comments cite D24, but the decision is D25.** The feature branch was cut before PR #240 merged and took D24 for manager blanket request authority. Docs are correct; the in-code citations need a sweep.
