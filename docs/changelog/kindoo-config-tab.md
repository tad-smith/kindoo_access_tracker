...
- **Superadmin-only editing.** The value is discovered automatically in the ordinary case and a wrong EID silently points every Kindoo operation at another environment. The rules gate was widened to match: `stakes/{stakeId}` update now allows `isManager(stakeId) || isBootstrapAdmin(stakeId) || isPlatformSuperadmin()`, so a superadmin can set a stake's home site without first being made a manager of it. Not field-scoped — it mirrors the read rule, which already exposes every stake's parent doc to superadmins, and an allowlist would need updating for every future Config-tab key. `lastActorMatchesAuth` and `validKindooConfig` still apply, and four rules tests pin that: the two positive paths (own stake, cross-stake) and the two invariants that must survive the widening.
