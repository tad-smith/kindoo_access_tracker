...
- **Delivery is one more drift code, not a backfill.** No callable, no migration script, no new authenticated surface, no second writer of the field. Stamping production is: run Sync, click through the rows, once per Kindoo site. The operator chose manual effort over permanent machinery deliberately. **The backlog is one-time; the code is not.** The check fires for every seat type and scope, while only a ward-scope `auto` primary is ever read back — so a new manual or temp seat still raises a row for a field nothing consults today. That is the requirement, not an oversight: a partially-populated provenance field is the future bug this feature exists to avoid, and narrowing the check to the seats that currently read it would reintroduce exactly that. At 1–2 requests/week the steady-state cost is a row every week or so. **This is the decision most likely to be "fixed" later by someone reading the drift row as a workaround — it isn't one.**
- **`buildings-mismatch` clears the stored subset in the same write.** The Church subset was observed against the old `building_names`, and the cascade guarantees the two codes never apply in the same Sync pass — so leaving it behind would let a newly Church-granted building render checked AND enabled, and a manager could edit the seat down to under-report until a later round-trip. Clearing to `null` restores "never observed", which locks. `applyTypeMismatch` clears it for the same reason, and there it is guaranteed wrong rather than merely stale — a manual seat can only ever hold `[]`, and PROMOTE turns that into an auto seat where `[]` unlocks everything. The rule is that any write touching `building_names` or `type` invalidates the subset.
