...
- **Pending-removal badge** — `partitionPendingForRoster` discriminates by composite key `(member_canonical, scope, kindoo_site_id)` via the exported `pendingRemoveKey` helper, so a pending remove on the East-Stake-Maple row doesn't dim the home-Maple row.
