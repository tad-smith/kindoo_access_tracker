...
**The two create fields also carry `WARD_NAME_BRANCH_WARNING`** — a red label under the input, shown live whenever `unitType()` reads the typed name as a branch. It uses the classifier's own predicate rather than a second regex, so it cannot disagree with what the save will do. Advisory only: `role="status"`, not wired to `formState.errors`, never blocks submit. It is the only thing standing between an operator naming a ward `Olive Branch` and the silent misconfiguration in **B-21**; Wards to Ignore deliberately does not show it, since a branch named there is ordinary.

**`parseDescription`'s lookup is now collision-safe.** `buildWardLookup` sorts units by `ward_code` (ascending, code-unit compare on a copy — `localeCompare` can rank distinct ids equal and would hand the tie-break back to array order) before registering variants, and a key already held by a different `ward_code` keeps its incumbent and warns once at `[sba-ext]`, naming both codes. The **sort** is what makes the winner independent of the order Firestore returns wards; first-wins and last-wins are both functions of array position. First-wins over the sorted list is still the better half of the pair, because a later unit keeps every variant the earlier one did not claim — so a branch sorting ahead of the ward that shadows it leaves both resolvable, where last-wins would strand the branch entirely. This matters for stakes that already hold a colliding pair, which the new guard prevents from being created but cannot retroactively repair.
...
- **B-21 — a lone ward in a place named "…Branch" is silently classified as a branch.** The field hint stops short of the `Olive Branch` / `Olive Branch Ward` case, an accepted operator call to keep it short, and the collision guard does **not** compensate: it needs two units to compare, so it only covers the stake holding both. The single-unit case — the stake's only unit there is a ward, the operator types `Olive Branch`, nothing collides — stores a branch. Two symptoms follow, not one: the provisioner's strict `!==` rewrites the church-provisioned Description on every pass, and the read side can no longer resolve it either — a branch registers only its verbatim variant, so `Olive Branch Ward (Bishop)` misses the lookup and Sync raises a `kindoo-unparseable` drift row whose remedy would reassign the members to stake scope. Partly mitigated by `WARD_NAME_BRANCH_WARNING` (above) — advisory only, so an operator who types past it still lands here. Recovery is a single rename to `"Olive Branch Ward"`, which fixes both sides; do it before repairing seats, or `unparseableAligned` flips back and the row returns.
