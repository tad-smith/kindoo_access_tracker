...
Worth noting for anyone reading the history: this is where Phase 6 originally landed. `docs/changelog/phase-6-request-lifecycle.md` records commit `982be4e` OR-ing `isManager(stakeId)` into the head of the create rule's role clause precisely because the then-current spec said managers submit in any scope, and a manager-only submit was failing. PR #52 removed it three days later, on the opposite reading. D24 restores the Phase 6 shape with the rationale written down this time.

Three sub-decisions, all recorded as `architecture.md` D24:
...
- `docs/architecture.md` — new **D24**, the full decision: reversal rationale, blanket-not-intersection, the superadmin exclusion and its deliberate divergence from `navModel.ts`, Policy 1 survival, the accepted ward-code non-guard, and the requester-display backstop. Cites D15 and D21.
...
- Nothing gated on this change. The ward-code existence gap (D24e) is accepted, not tracked — file a task only if a bad scope ever reaches production data.
