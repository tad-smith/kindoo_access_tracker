...
- **Stake Roster.** Each card's top line shows an org chip ("{name}" / "No Organization") as a neutral-grey Badge-style pill (matching the seat-type chips beside it, no "Org:" prefix). The editable pill carries a small ▾ caret and overlays a transparent native `<select>`, so a click anywhere on it opens the menu; selecting an org writes the seat immediately — a direct client write, the **only** client seat-update path. Editable by anyone with stake app access (`stakes[stakeId].stake == true`), targeting the **primary stake grant** only; a parallel-site stake *duplicate* grant's chip is read-only (a plain grey pill, no caret — org set via the request form). Below the stake bar, one committed bar per organization; each utilization bar shows its label to the **left of the bar** (name | bar | count), so the per-org bars read "{name}" on the left and the committed stake bar's left label is **"Stake Total"**; the org cap drives ok / warn ≥90% / over but never blocks.
