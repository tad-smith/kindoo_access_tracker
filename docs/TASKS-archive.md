...
**Backend.** `sendManualSeatReviewIfDue(stakeId, now)` (`functions/src/services/ManualSeatReviewService.ts`) follows D37's shape: a per-stake unit of work with no trigger, registered in `SCHEDULED_JOBS` as `manualSeatReview`, `{type:'monthly', day:1, hour:2}`, `jitterSeconds: 72_000`, `defaultEnabled: false`. **Deliberately no `quarterly` schedule shape** — `MANUAL_SEAT_REVIEW_INTERVAL_DAYS = 75` in the handler is what decides whether anything sends, and 75 sits in the middle of the 63–90 day band that rejects every second month and admits every third, forever, for a check landing on the 1st (a stamp set off-cycle runs one cycle long, then self-corrects). Jitter (20 hours) is a deterministic per-stake offset (`jitterDelaySeconds`, FNV-1a over `${stakeId}--${job}`), not random, so a stake's send time is reproducible and two jittered jobs on one stake can't collide.
