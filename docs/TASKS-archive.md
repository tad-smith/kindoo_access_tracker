...
Closed entries moved out of [`TASKS.md`](TASKS.md) so the live file stays small enough for every session and every agent to read in full. Nothing here is active work — don't pick something up from this file.

Entries keep their original heading, status line, and body verbatim. An `obsolete` entry in particular keeps the body it had, because the premise that turned out not to hold is the part worth reading. Numbers are never reused, so `git grep T-NN` finds an entry whether it lives here or in `TASKS.md`.

Format is unchanged — see the header of `TASKS.md`.
...
**Backend.** `sendManualSeatReviewIfDue(stakeId, now)` (`functions/src/services/ManualSeatReviewService.ts`) follows D37's shape: a per-stake unit of work with no trigger, registered in `SCHEDULED_JOBS` as `manualSeatReview`, `{type:'monthly', day:1, hour:2}`, `jitterSeconds: 72_000`, `defaultEnabled: false`. **Deliberately no `quarterly` schedule shape** — `MANUAL_SEAT_REVIEW_INTERVAL_DAYS = 75` in the handler is what decides whether anything sends, and 75 sits in the middle of the 63–89 day band that rejects every second month and admits every third, forever, for a check landing on the 1st (a stamp set off-cycle runs one cycle long, then self-corrects). Jitter (20 hours) is a deterministic per-stake offset (`jitterDelaySeconds`, FNV-1a over `${stakeId}--${job}`), not random, so a stake's send time is reproducible and two jittered jobs on one stake can't collide.
...
**Done.** Shipped as scoped: the extension writes `syncHeartbeats/{stakeId}/sites/{siteKey}` unconditionally on every completed scan (`SyncPanel.tsx` → `writeSyncHeartbeat`), and `sendSyncReminderIfDue` evaluates it as a second, independent condition alongside T-103's expired-seat check, sending one mail naming whichever fired. Both settled questions above landed as decided: an absent heartbeat is silent (never chased), and there is no "last sync" display — the bonus floated in the last paragraph below was not built. `SYNC_STALE_DAYS = 7` lives in `SyncReminderService.ts`, not `@kindoo/shared`. No new registry entry, no new push category, no new manager-facing control — the existing `syncReminder` job and toggle now cover both conditions. `SyncReminderOutcome.status`'s `nothing-expired` renamed to `nothing-due`, and the type gained `staleSites: number` — a breaking rename of the return literal, not additive. Recorded as `architecture.md` D40, amending D37 in place a third time. The Configuration-page `InfoTip` and the Notifications-page push label described only the expired-seat condition, and the `InfoTip`'s "stops on its own once they are gone" had become wrong once the stamp began surviving until both conditions clear; both were corrected in this PR rather than deferred, since this PR is what made them false. See `docs/changelog/sync-reminder-heartbeat.md`.
...
**Flow.** Hourly dispatcher → for each stake, each enabled trigger whose `next_trigger_time <= now` → enqueue a Cloud Tasks task naming the trigger's queue job and the stake → stamp `last_trigger_time = now` and recompute `next_trigger_time`. One generic `onTaskDispatched` runner resolves the handler from a registry keyed by job name; handlers receive `{stakeId}` and nothing about scheduling.

**Design settled 2026-09-05, operator-approved.** This replaces the open questions the entry originally carried; where a decision here contradicts an earlier line above, this section wins.

- **Where the array lives.** A new **top-level** `stakeSchedules/{stakeId}` doc holding `tasks: ScheduledTask[]`. Not the stake parent doc: the hourly stamp would fan an `auditStakeWrites` row every fire, and silencing that via `BOOKKEEPING_FIELDS` would also silence a manager toggling `enabled` — the one part worth auditing. Not a sub-collection of `stakes` either, because `auditTrigger.registration.test.ts` derives the audited set from the rules file's `stakes/{stakeId}` slice, so a block there forces either an audit trigger or an `UNAUDITED` entry. Top-level sits outside that slice by design, the way `remoteApply` does. **Accepted consequence: writes to `stakeSchedules` are not audited at all.**
- **Manager writes reach the timing fields, and that is accepted.** Rules cannot see inside array elements, so `isManager(stakeId)` covers `next_trigger_time` as much as `enabled`. Not worth splitting into config and state docs: the worst case is a manager making their own stake's own job fire early, at most once an hour, against a handler that carries its own backoff.
- **One generic runner, not one function per job.** `runScheduledTask` takes `{stakeId, job}` and resolves the handler from an in-process registry, so the system stays two deployed functions and one queue permanently and a new scheduled feature is a registry entry. Supersedes this entry's original "handlers are `onTaskDispatched` functions", which would have added a function and a queue per feature — the per-feature cost this task exists to remove.
- **Missed windows advance from the stored value, never from `now`.** `nextTriggerTime` starts at the stored `next_trigger_time` and advances one slot at a time until it lands in the future. Drift is prevented by the slots themselves: Every shape's slots are anchored to a wall-clock boundary — `hourly` to the top of the hour, the other three to their configured hour — so no trigger can inherit the second at which some dispatch happened to run. It still fires once and re-bases rather than replaying every slept-through window, because the loop stops at the first future slot; the loop is bounded so a long-stale or hand-typed value cannot spin.
- **Cloud Tasks dedupe, and exactly how far it reaches.** Deterministic task id per (stake, job, hour), with `task-already-exists` caught as success. Cloud Tasks holds a used id for roughly an hour after execution, which matches the dispatch cadence exactly. **This dedupes task *creation* only.** Once the one task exists, Cloud Tasks re-dispatches it under the same id whenever an attempt misses its deadline, and it does not cancel the attempt still in flight — so it does not serialise two overlapping *executions*. It therefore narrows the hazard inherited from [T-103] below without closing it; that one stays open for the wiring PR.
- **Enqueue first, stamp second.** The other order loses a fire silently. The dispatcher is therefore **at-least-once**, and handlers must be idempotent within their own window.
- **Calendar edges.** `monthly` past a month's length clamps to the last day of that month; hour-of-day resolves through the noon-sampled offset `packages/shared/src/stakeTime.ts` already uses.
- **Seeding belongs to the dispatcher, and defaults are OFF.** Each hour it adds an entry for any registry job a stake lacks, never resurrecting or overwriting an existing one — so a manager's `enabled: false` survives. That covers a brand-new stake and a newly shipped job alike, and replaces the `createStake` seeding plus one-row backfill this entry originally recorded. `defaultEnabled` is `false` for every job: **explicit opt-in**, so a seeded job never starts mailing anyone until someone turns it on. With no manager UI yet, that flip is a Firestore console edit.
- **Infra.** The Cloud Tasks queue is auto-provisioned by the Firebase CLI at deploy. The dispatcher re-establishes the scheduled job [T-102] removed — `spec.md` §2, `architecture.md` D35, and root `CLAUDE.md` all assert zero scheduled Cloud Functions and all need amending. Cloud Scheduler's free tier is then fully spent: staging dispatcher, prod dispatcher, prod weekly export — 3 of 3, on a per-billing-account limit. That is the point rather than a problem: every future scheduled *feature* costs nothing, because it is a row in an array.

**First consumer, already merged.** The sync reminder ([T-103]) shipped 2026-09-04 with no trigger of its own; a third PR registers `sendSyncReminderIfDue` as a queue job. The operator split delivery three ways — reminder, then cron, then the wiring — so **this task's PR ships the dispatcher with an empty job registry**. That ordering was deliberate — the reminder proves the handler contract before the dispatcher exists to call it — and it is also a debt: until this lands, `functions/src/services/SyncReminderService.ts` is code nothing runs.

**Inherited from T-103.** The reminder's `last_sync_reminder_date` backoff stamp dedupes **sequential** redelivery only. Two invocations for the same stake overlapping in time both read the pre-write stamp and both send. At one dispatch per stake per hour that needs a retry landing on top of a still-running attempt, so it is unlikely rather than impossible. **Still open after [T-104]**, deliberately: deterministic task names dedupe enqueue, not execution (see the dedupe bullet above), so the remedy is a lease or a transactional compare-and-set on the backoff stamp. Decide it in the PR that registers the reminder, since that is the first handler it can actually bite.
...
Status: done (2026-08-18 — PR #287)
Owner: @backend-engineer (constants + backfill) + @infra-engineer (runbook) + @web-engineer (privacy policy + fix registration)
Phase: cross-cutting

**Done.** `AUDIT_TTL_MS` in `packages/shared/src/types/audit.ts` is the single definition of the five-year window, replacing the three private `TTL_MS` literals; `createStake`'s copy went away with the field it stamped. `platformAuditLog` rows carry no `ttl` at all — `PlatformAuditLog.ttl` is optional in the type and the zod schema, pre-existing rows keep their stamped-but-unenforced value, and §3.3 / §5.2 of `firebase-schema.md` now say a TTL policy must never be enabled there. The `backfillAuditTtl` superadmin callable restamps existing rows from each row's own `timestamp`, skip-if-equal, on a `BulkWriter` with a projected read; it is registered as **Backfill audit log retention** in the Stake List Apply Fixes menu and **must be run once per stake by the operator** — it is not automatic, and it is only meaningful before ~2027-05. The runbook gained §1.6.1 (the TTL-policy step [T-15] asked for in 2026-04-29 and never got) and §5.4.1 (a verification that `auditLog` is the only collection group with a policy). **The premise that "the 365-day value has no recorded rationale anywhere" was wrong, and this is the part worth remembering:** it was recorded — in the published privacy policy, which told members audit records "are retained for accountability purposes for 365 days, after which they are deleted." The task's author looked in `F` / `D` decisions and in code and found nothing, because the rationale was a commitment to the people named in the rows, living in `apps/web/src/routes/privacy.tsx`. Extending retention therefore meant republishing that promise (operator-approved; `LAST_UPDATED` bumped), not editing a constant. Recorded as `architecture.md` D36, which is also where Q20 resolves. See `docs/changelog/extend-audit-retention.md`.
