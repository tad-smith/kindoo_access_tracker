...
But the surface area is non-zero. If a future Chrome / Safari / Firefox change makes embedded sign-in flows unusable (e.g. requiring Storage Access API prompts the user can't reasonably approve), the documented fallback is to drop iframe-embed and accept the banner — `Config.main_url` reverts to the raw `/exec` URL, `website/index.html` becomes a redirect rather than a wrapper, and we accept the banner. Cloudflare-Worker-as-transparent-proxy is the second-fallback if even the redirect approach has UX problems.
...
**Flagged (PR #130 review, 2026-05-17).** When a `duplicate_grants[]` entry's `scope` points at a ward that has been deleted (or otherwise can't be looked up), what's the right behaviour: error the whole migration, fall back to "home", or skip the entry?
...
**Resolved 2026-05-17.** Option (b): functional. Clicking Remove on a duplicate row generates a `remove` request whose `scope` and new `kindoo_site_id` field reflect the duplicate's grant, not the seat's primary. When the request is marked complete only that `duplicate_grants[]` entry is removed; the primary and any other duplicates stay intact; the Kindoo removal write fires against the correct foreign site via the Phase A per-site orchestrator. Spec consequences: (i) a new optional `kindoo_site_id?: string | null` field on the `remove` request schema in `packages/shared/src/schemas/request.ts`, (ii) `planRemove` in `functions/src/triggers/removeSeatOnRequestComplete.ts` (lines 77-94) keys on `(scope, kindoo_site_id)` rather than `scope` alone — with scope-only fallback for legacy remove requests that predate Phase B, (iii) the `firestore/firestore.rules` `requests.create` predicate already accepts arbitrary additional fields, so no rule change is required (defense-in-depth `kindoo_site_id is string` type-check is implementer's call). The provision orchestrator is unchanged (Phase A already routes per-grant by `kindoo_site_id`). Note: the scope-aware remove logic lives in the trigger, not in `markRequestComplete.ts` — the callable's remove branch only reads `seatSnap.exists` and flips the request to `complete`; the trigger does the actual `duplicate_grants[]` walking. Follow-up: same-scope same-site disambiguator (KS-9) is open; `(scope, kindoo_site_id)` matching collapses to scope-only when both grants are on the same site.

### KS-9 `[RESOLVED 2026-05-17 — per-grant UUID not needed; (scope, kindoo_site_id) is sufficient. Within-site collision losers are surfaced via the existing priority rules; the Remove path operates on the (scope, kindoo_site_id) tuple which is unique at the moment of removal.]` T-42 Phase B — disambiguator for same-scope same-site duplicate-row Remove

**Flagged (T-42 Phase B spec PR, third-pass review 2026-05-17).** Spec §15 Phase B AC #2 requires that a seat with primary `scope='Cordera'` + duplicate `scope='Cordera'` renders 2 rows on AllSeats. AC #8 requires that clicking Remove on the duplicate row splices only that duplicate-grant entry, leaving the primary intact. But under Phase A's data model a within-site priority loser carries `kindoo_site_id === primary.kindoo_site_id`. Phase B's proposed `(scope, kindoo_site_id)` disambiguator in `planRemove` therefore collapses to scope-only matching for the same-scope same-site case, which would delete-or-promote the primary instead of splicing the duplicate. The same problem applies retroactively to any in-flight pre-Phase-B remove request created against a seat that has acquired a within-site duplicate post-Phase-A.

**Options.**
- **(a) Array index on the `remove` request.** Add `duplicate_grants_index: number | null` to the remove-request schema. Client populates it from the duplicate row; `planRemove` reads it. Brittle if the array is rewritten between request-creation and trigger-firing (importer can renumber on the next run).
- **(b) Per-grant UUID on `duplicate_grants[]`.** Phase A's data model gains a `DuplicateGrant.id: string` field. Importer + `markRequestComplete.planAddMerge` write it on every new duplicate. Remove request carries `duplicate_grant_id: string | null`; `planRemove` matches on the id when present, falling back to `(scope, kindoo_site_id)` then scope-only. Stable across array rewrites; small Phase A scope addition.
- **(c) Composite `(scope, kindoo_site_id, type)`.** Use `type` (auto/manual/temp) as a tiebreaker. Only works if same-scope same-site duplicates always differ on `type`, which Phase A doesn't guarantee.
...
**Options.**
- **(a) Drop the `where` clause; fetch the full seats collection per page.** Works at the 250-seat scale; burns reads × wards on every page visit. Simple; no new index.
