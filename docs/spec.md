...
Static `docs/index.html` page hosted on GitHub Pages, custom-domained to `kindoo.csnorth.org`. The page is a full-viewport iframe whose `src` is the Main Apps Script `/exec` URL; both `doGet` deployments (Main + Identity) set `HtmlService.XFrameOptionsMode.ALLOWALL` so the cross-origin embed is permitted. `Config.main_url` holds `https://kindoo.csnorth.org` so the Identity → Main redirect lands users back on the wrapper origin. DNS is a single `CNAME kindoo → <github-username>.github.io` at Squarespace; HTTPS is auto-provisioned by GitHub Pages via Let's Encrypt. See `architecture.md` §11 for the full architecture (including why the iframe-embed approach was chosen over the originally-planned Cloudflare Worker proxy: a transparent proxy would deliver the pretty URL but cannot remove the Apps Script banner).
...
11. **Custom domain via iframe wrapper** — last, since the Apps Script URL works fine during development. Static `docs/index.html` on GitHub Pages, full-viewport iframe to Main `/exec`, ALLOWALL on every `doGet` HtmlOutput. (Originally planned as a Cloudflare Worker proxy; pivoted because a proxy can't remove Apps Script's banner — see `architecture.md` §11.)
