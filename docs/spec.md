...
The distinguishing test is field equality on `kindoo_site_id`; no new flag is needed.
...
- Pick the primary by existing priority rules (lowest `sheet_order`, etc.); write `scope`, `building_names`, and the derived `kindoo_site_id` at top level.
- For every **other site** the person has a calling on, emit one entry in `duplicate_grants[]` carrying that site's `kindoo_site_id`, the calling list scoped to that site, and the buildings derived from those callings' wards (or the stake-scope template when applicable).
- Within-site priority losers continue to land in `duplicate_grants[]` with `kindoo_site_id === primary.kindoo_site_id` — today's behaviour is preserved unchanged.
...
**Provision orchestrator.** Per-site writes. When a request or auto-seat needs provisioning, the orchestrator emits one Kindoo write per distinct `kindoo_site_id` across (primary + parallel duplicates), each using the matching Kindoo session. Within-site priority losers (same `kindoo_site_id` as primary) do not get a separate write — the primary's write already covers them. The EID-check (already in place) keys off each grant's `kindoo_site_id`.

**Utilization.** Shape unchanged: foreign-site ward seats stay excluded from home-stake utilization. The calculation reads `Seat.kindoo_site_id` directly instead of re-deriving from the seat's ward each time.

**Request completion auto-merge.** When `markRequestComplete` writes a manual/temp grant against an existing seat, the new `duplicate_grants[]` entry must stamp `kindoo_site_id` (derived from the request's scope and ward). `building_names` was already recorded on the duplicate; this adds the site.

**One-shot migration.** A separate migration step backfills `kindoo_site_id` on every existing primary seat and every existing `duplicate_grants[]` entry by looking up the entry's `scope` → ward → `kindoo_site_id` (stake-scope ⇒ home). Run via a one-time admin callable or scripted batch; specified at implementation time.
