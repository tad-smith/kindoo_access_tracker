...
**Server-side surface.** Phase B requires two server-side changes — one to enable the broadened-inclusion reads on roster pages, one to support the functional Remove button on duplicate rows:

- **`firestore/firestore.rules` — widen `seats.read`.** Today's `seats.read` rule (`firestore.rules:432-434`) permits a bishopric to read a seat only when `resource.data.scope in bishopricWardOf(stakeId)` — i.e., on the seat's primary scope. The broadened-inclusion rule ("any grant matches") requires bishopric and stake-presidency reads to extend to seats whose primary scope is some other ward (or `'stake'`) but whose `duplicate_grants[]` includes an entry whose scope matches the reader's. The rule will widen to `(resource.data.scope in bishopricWardOf(stakeId)) || (any duplicate's scope ∈ bishopricWardOf(stakeId))` — modelled as a Firestore-rules predicate using array membership over `resource.data.duplicate_grants[*].scope`. Same widening applies to the `isStakeMember` clause if a stake-presidency member needs to read a seat whose primary is a ward and whose duplicate is stake-scope. Without this rules change, the client-side widened filter has nothing to filter — the wider query just returns the same primary-matched set it does today.
