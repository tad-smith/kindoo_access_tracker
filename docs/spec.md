...
- **Auth**: Google Sign-In (GSI) from the client; server-side JWT verification of the ID token against Google's JWKS on every `google.script.run` call. The verified `email` claim — canonicalised (lowercase + Gmail dot/`+suffix` stripping + `googlemail.com` → `gmail.com`) — is resolved against the `KindooManagers` and `Access` tabs. Users sign in with consumer Gmail accounts, so the OAuth consent screen is Published and the web app's `access` is `ANYONE_WITH_GOOGLE_ACCOUNT`.
