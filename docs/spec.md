...
- **New denormalized `Seat.duplicate_scopes: string[]` field — OWNED BY PHASE A, GATING PHASE B.** Firestore-rules CEL cannot project a field across an array of objects (no `array[*].field` projection; only primitive-array operations like `in` / `hasAny` / `hasAll`), so the bishopric / stake-presidency `seats.read` widening cannot read `duplicate_grants[].scope` directly. The fix is to maintain a denormalized mirror — `Seat.duplicate_scopes: string[]` — that lists every `duplicate_grants[].scope` value. The field is written server-side wherever `duplicate_grants[]` is written: Phase A's importer fan-out, `markRequestComplete.planAddMerge`, and the T-42 one-shot migration backfill. The field is owned by Phase A's data model + writers + migration; the Phase A implementation PR will be updated to land it alongside `kindoo_site_id`. **T-43 cannot ship without it** — the rules widening below depends on it. The Phase A "Multi-site grants — data model" subsection will be updated by the Phase A implementation PR to document this field; this Phase B subsection cross-references it.
- **`firestore/firestore.rules` — widen `seats.read` against `duplicate_scopes`.** Today's `seats.read` rule (`firestore.rules:432-434`) permits a bishopric to read a seat only when `resource.data.scope in bishopricWardOf(stakeId)` — i.e., on the seat's primary scope. The broadened-inclusion rule ("any grant matches") becomes `resource.data.scope in bishopricWardOf(stakeId) || resource.data.duplicate_scopes.hasAny(bishopricWardOf(stakeId))`. Symmetric widening for the stake-presidency clause if a presidency member needs to read a seat whose primary is a ward and whose duplicate is stake-scope. Without this rules change, the client-side widened filter has nothing to filter — the wider query just returns the same primary-matched set it does today.
...
10. **`seats.read` rule widened for any-grant scope match.** A bishopric member of ward X can read a seat whose primary is `scope='stake'` (or some other ward) and whose `duplicate_grants[]` includes an entry with `scope='X'` (mirrored on `duplicate_scopes`). Symmetric widening for stake-presidency reads against seats with stake-scope duplicates whose primary is a ward. Verified by `firestore/tests/` rules unit tests covering both bishopric-via-duplicate and presidency-via-duplicate read paths, plus a negative test that a non-matching reader is still denied.
11. **`Seat.duplicate_scopes` denormalized field present and maintained.** Every Seat doc carries `duplicate_scopes: string[]` mirroring `duplicate_grants[].scope`; written by the importer fan-out, by `markRequestComplete.planAddMerge`, and by the T-42 migration backfill. Owned by Phase A; Phase B will not ship until Phase A's PR includes the field on every code path that writes `duplicate_grants[]`. Verified by Cloud Functions integration test against the importer and `markRequestComplete`, plus a migration test asserting the field is populated on every seat after a one-shot run.
