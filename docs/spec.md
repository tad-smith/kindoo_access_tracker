...
**Default landing rule.** Multi-role principals resolve via priority — manager > stake > bishopric — and land on the most-privileged role's default page. Per-role defaults: manager → `/manager/dashboard`; stake → `/stake/roster`; bishopric → `/bishopric/roster`. Non-Kindoo-Manager roles intentionally land on the Roster (not the leftmost nav tab) so the first surface those users see is the current seat list for their scope. The bishopric Roster header carries a "New Request" button next to the ward name that links to `/new` (§5.1); the stake Roster does not carry this affordance — stake users use the "New Request" nav link instead.
...
Same three pages as Bishopric, scoped to the stake pool — uses the same shared form / list components. The Stake Roster header does NOT carry the bishopric Roster's "New Request" button; stake users use the "New Request" nav link instead. Plus:
