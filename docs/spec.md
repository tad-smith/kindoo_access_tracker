...
- **Domain**: `kindoo.csnorth.org`. A static `website/index.html` page deployed to GitHub Pages (via the Actions workflow at `.github/workflows/pages.yml`) contains a full-viewport iframe pointing at the Main Apps Script `/exec` URL; both `doGet` deployments set `setXFrameOptionsMode(ALLOWALL)` to permit the embed. Net effect: pretty URL in the address bar AND the *"This application was created by a Google Apps Script user"* banner is hidden (the banner lives in Apps Script's outer wrapper page, which we no longer load at the top frame). See `architecture.md` §11.
...
Static `website/index.html` page deployed to GitHub Pages via the Actions workflow at `.github/workflows/pages.yml`, custom-domained to `kindoo.csnorth.org`. The page is a full-viewport iframe whose `src` is the Main Apps Script `/exec` URL; both `doGet` deployments (Main + Identity) set `HtmlService.XFrameOptionsMode.ALLOWALL` so the cross-origin embed is permitted. `Config.main_url` holds `https://kindoo.csnorth.org` so the Identity → Main redirect lands users back on the wrapper origin. DNS is a single `CNAME kindoo → <github-username>.github.io` at Squarespace; HTTPS is auto-provisioned by GitHub Pages via Let's Encrypt; the custom domain is preserved in the published artifact via `website/CNAME`. See `architecture.md` §11 for the full architecture (including why the iframe-embed approach was chosen over the originally-planned Cloudflare Worker proxy: a transparent proxy would deliver the pretty URL but cannot remove the Apps Script banner).
...
11. **Custom domain via iframe wrapper** — last, since the Apps Script URL works fine during development. Static `website/index.html` on GitHub Pages (deployed via `.github/workflows/pages.yml`), full-viewport iframe to Main `/exec`, ALLOWALL on every `doGet` HtmlOutput. (Originally planned as a Cloudflare Worker proxy; pivoted because a proxy can't remove Apps Script's banner — see `architecture.md` §11.)
