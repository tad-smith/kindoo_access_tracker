...
**Remove button on duplicate rows in AllSeats — functional, scoped to the duplicate's (scope, kindoo_site_id).** Clicking Remove on a duplicate row generates a `remove` request whose `scope` and new `kindoo_site_id` field reflect the duplicate's grant, not the seat's primary. The `<RemovalAffordance>` component takes an optional `grant` prop; AllSeats passes it on every per-grant row. The `useSubmitRequest` mutation stamps `kindoo_site_id` on the request body when the caller sets it; the `removeSeatOnRequestComplete` trigger uses it to splice the right entry.
...
- **Optional `kindoo_site_id` on remove requests.** `packages/shared/src/schemas/request.ts` + `packages/shared/src/types/request.ts` add `kindoo_site_id?: string | null`. The field is present on requests generated from a duplicate row; absent on remove requests generated from a primary row (legacy / today's path). The `requests.create` rule predicate accepts arbitrary additional fields, so no rules change is required for the new field.
- **`planRemove` in `functions/src/triggers/removeSeatOnRequestComplete.ts`.** Keys on `(scope, kindoo_site_id)` when the request carries `kindoo_site_id`; falls back to scope-only matching when the field is absent (legacy / primary-row removes). Primary-row remove with `(scope, kindoo_site_id)` matching the primary's: today's delete-or-promote path applies. Duplicate-row remove: only the matching `duplicate_grants[]` entry is spliced; primary + remaining duplicates stay intact, and the Kindoo removal write fires against the correct foreign site via the Phase A per-site orchestrator.
