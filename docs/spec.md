...
**`actionCodeSettings`.** `url` is the full SPA action-handler URL (must include the host) and the host must be on Firebase's Authorized Domains list (Console → Auth → Settings → Authorized domains; both `stakebuildingaccess.org` and `kindoo.csnorth.org` qualify per §12). `handleCodeInApp: true`. Adding a new authorized SPA host is a deployment prerequisite, not a runtime concern.
...
- For every **(scope, kindoo_site_id)** combo that isn't the primary, the importer emits one entry in `duplicate_grants[]` carrying that site's `kindoo_site_id`, the scope, the calling list for that scope, and the buildings derived from that scope. Two foreign wards on the same foreign site produce two `duplicate_grants[]` entries, both with that site's `kindoo_site_id` but distinct `scope` values; the sync detector unions their `building_names` per-site when computing expected buildings.
- Importer-written parallel-site duplicates always set `building_names` (derived from the duplicate's `scope` → ward → `building_name`, or the stake-scope home-buildings list for stake duplicates). Within-site importer duplicates leave the field unset and inherit from the primary's ward.
...
**Utilization.** Foreign-site ward seats stay excluded from home-stake utilization. The calculation reads `Seat.kindoo_site_id` directly when populated (the importer, `markRequestComplete`, and the migration stamp it) and falls back to the seat's `scope` → ward `kindoo_site_id` for legacy seats still in the migration window. Externally observable behaviour is unchanged — the field is a denormalisation.
...
**AllSeats — multi-row per seat.** `apps/web/src/features/manager/allSeats/AllSeatsPage.tsx` renders one row per grant: the primary plus one row per `duplicate_grants[]` entry (both within-site priority losers and parallel-site duplicates). Each row's columns — scope, callings, type, building_names, foreign-site badge, reason / dates for manual / temp — reflect the grant being rendered, not the seat's primary alone. The pending-removal badge query (`partitionPendingForRoster` in `apps/web/src/features/requests/rosterPending.ts`) discriminates by `(member_canonical, scope, kindoo_site_id)` via the exported `pendingRemoveKey` helper, so a pending remove on one row doesn't light up another row with the same scope but a different `kindoo_site_id`. Two same-scope rows on the same `kindoo_site_id` (a true within-site collision case) still both light up — both are the same logical grant from Kindoo's perspective.
...
**Same-scope priority losers.** Stay invisible on per-scope roster pages (Bishopric Roster, Stake Roster, Ward Rosters). AllSeats renders same-scope priority losers as their own rows under the multi-row rule; they are informational only — no separate Kindoo write happens for within-site losers; the primary's write already covers them.
...
**Edit button on duplicate rows in AllSeats — disabled with tooltip.** The Edit button renders on every row to preserve the action-column rhythm but is disabled on duplicate rows (any row backed by a `duplicate_grants[]` entry, including same-scope within-site losers). The tooltip is rendered per case to avoid misleading wording on within-site losers:
...
- **Within-site duplicate row** (`isParallelSite === false`): *"Edit the primary grant to modify this person's seat — this row is informational and is covered by the primary's write."*
...
The Remove affordance does NOT render on a same-`(scope, kindoo_site_id)` within-site priority loser when the primary is itself non-auto (manual / temp). Submitting from such a row would carry the same `(scope, kindoo_site_id)` tuple as the primary, and the trigger's discriminator would target the primary — silently demoting / removing the wrong grant. Those rows stay informational only per §412. There is one exception: when the primary is `auto` and a non-auto duplicate sits at the same `(scope, kindoo_site_id)` (a rare case reachable via `planAddMerge` when an `add_manual` lands on a person already holding an LCR-importer auto seat), the Remove affordance does render on the duplicate row. The trigger's KS-9 auto-primary disambiguation in `planRemove` (`functions/src/triggers/removeSeatOnRequestComplete.ts`) splices the non-auto duplicate and leaves the auto primary intact, because an auto grant can never have been the user's intended removal target.
...
- **`Seat.duplicate_scopes: string[]` denormalized field (OWNED BY PHASE A).** Firestore-rules CEL cannot project a field across an array of objects (no `array[*].field` projection; only primitive-array operations like `in` / `hasAny` / `hasAll`), so the bishopric `seats.read` widening reads against this primitive mirror. Phase A maintains it on every seat-write path: importer fan-out, `markRequestComplete.ts` (fresh-seat create + `planAddMerge` merge branches), `syncApplyFix.ts`, `removeSeatOnRequestComplete.ts` (both promote and drop_duplicate paths), the web-side queue-completion seat write, and the T-42 migration backfill.
...
8. **Remove on duplicate row — functional when the discriminator differs from the primary.** Clicking Remove on a parallel-site or cross-scope duplicate row generates a `remove` request scoped to **that duplicate's (scope, kindoo_site_id)**. When marked complete, only that `duplicate_grants[]` entry is removed; primary + remaining duplicates stay intact; the Kindoo removal write goes to the correct foreign site. A same-`(scope, kindoo_site_id)` within-site priority loser under a non-auto primary renders without a Remove affordance (informational only per §412). The auto-primary + non-auto-duplicate case at the same `(scope, site)` renders Remove; the trigger's KS-9 disambiguation routes the splice to the non-auto duplicate. Verified by Cloud Functions integration test + RTL tests for both reachable cases and the informational-only hidden case.
