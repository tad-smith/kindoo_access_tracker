...
- **A job's per-stake special case lives on the registry entry, never in the dispatcher.** `ScheduledJob.skipJitter?: (stake) => boolean` is the pattern (`manualSeatReview` waives its jitter under `stake.manual_seat_review_dry_run`, D42): the job supplies the predicate, the dispatcher calls it and **names no job's field**. The moment `dispatchScheduledTasks.ts` reads one job's field directly, every future job's special case has a precedent. Any such predicate runs against a hand-edited document, so the dispatcher catches a throw, logs it at ERROR, and falls back to the default rather than stranding that stake's other jobs. Keep `jitterDelaySeconds` pure — "stake X runs at second N, always" is what makes an incident review possible.
