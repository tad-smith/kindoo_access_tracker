...
4. `docs/firebase-migration.md` — phase plan (Phase A complete; Phase B active).
...
- **`{stakeId}` parameterized from day one** (per F15). The hardcoded `'csnorth'` constant in `apps/web/src/lib/constants.ts` is going away in Phase B's B.4 PR in favour of the active-stake selector — every new per-stake read consumes the active stake, not the constant.
...
- Phase B (multi-stake) — **active** as of 2026-05-18; ships as five sub-deliverables (B.1 → B.5). See `docs/firebase-migration.md` Phase B and T-46.
