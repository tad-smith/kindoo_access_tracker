...
- **Sync reminder wiring** (PR #290, `architecture.md` D39, `spec.md` §9 / §17, T-107) — shipped. `SCHEDULED_JOBS` registers its first entry, `syncReminder → sendSyncReminderIfDue`, `{type:'daily', hour: 6}`, `defaultEnabled: false`. The Configuration page's Config tab carries a three-row slider stack below `Save config`: Email Notifications Enabled, then Sync reminders nested beneath it as a sub-row, then Elders Quorum Presidents Get App Access — each writes immediately on flip (no Save button covers any of the three), and every explanation lives in a tap-triggered `InfoTip` beside its row rather than in the page body. **`notifications_enabled` is email-only, and greying the Sync reminders row is not the same as pausing it** — the row disables when its parent is off or when it's unseeded, but the reminder still honours its own `enabled` value: push and the three-day backoff stamp both still happen, only the send is skipped, and the row's tip says so explicitly so the grey isn't read as dormant. **Enabling can mail within the hour, or not, and the tip states which before the flip**: a disabled row is never stamped, so a stale `next_trigger_time` makes it due at once — but a freshly-seeded row (every stake gets one at the next local 06:00 on rollout) is usually not stale yet, so enabling it that afternoon waits for tomorrow's 06:00 instead. **Registering the job exposed two real bugs, both fixed in this PR rather than filed.** First, a clobber race in the dispatcher's stamp: the per-stake write is now a transaction (`commitScheduleChanges`) that re-reads `stakeSchedules/{stakeId}` and re-applies only that pass's computed stamps and seeded rows, so a manager's concurrent `enabled` toggle survives a dispatch pass instead of being silently reverted. Second, a silent email failure: `runScheduledTask` is now the first scheduled handler that can send mail, and it declared no secret — `RESEND_API_KEY` was unset at runtime, the send failed inside `sendOne`, and `sendSyncReminderIfDue` still reported `status: 'sent'` and consumed the backoff. Fixed by binding `secrets: [RESEND_API_KEY]` on `runScheduledTask`, the same way the three notify triggers already do. Stamp-last stands, unchanged and deliberate: delivery is at-least-once, so two overlapping executions can both mail, which is preferred to the alternative of claiming the stamp first and risking up to three days of silence about seats that are actively expired. See `docs/changelog/sync-reminder-wiring.md`.
