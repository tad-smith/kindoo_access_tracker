...
- **Expired temp seats** (`architecture.md` D34, `spec.md` §7) — shipped. A temp grant past its `end_date` renders muted with an `Expired` badge and **no Remove control**, replaced by a line saying the seat clears at the next Sync. Kindoo Managers keep the control and get the badge on All Seats. It is marked, **not hidden** — `end_date` mirrors what was provisioned, not what Kindoo currently holds, so hiding could conceal a still-live seat. Display-only; the rule lives in `apps/web/src/lib/tempExpiry.ts` and nowhere else. See `docs/changelog/expired-temp-seat-display.md`.
